...
TOP_IMAGES = {
    "points": "top_scorers.png",
    "goals": "top_snipers.png",
    "assists": "top_assistants.png",
    "goalkeepers": "top_goalkeepers.png",
}
FONT_PATH = "DejaVuSans-Bold.ttf" 

# 🔧 ИЗМЕНИТЕ ЭТО ЧИСЛО, ЧТОБЫ ПОДОБРАТЬ РАЗМЕР ШРИФТА
FONT_BASE_SIZE = 38 

# 🔧 Ширина ваших полосок в пикселях (чтобы текст не выходил за рамки)
BOX_WIDTH = 800 

# 🔧 Координаты (X, Y) центров 9 строк. УБРАНО 10-е место!
ROW_COORDS = [
    (540, 200), (540, 280), (540, 360), (540, 440), (540, 520),
    (540, 600), (540, 680), (540, 760), (540, 840)
]
...
def create_top_image(stat_key: str, players: list) -> io.BytesIO:
    ...
    def _fit_font(text, max_width, base_size):
        """Уменьшает шрифт, если текст не влезает в ширину полоски."""
        size = base_size
    ...
    # Меняем срез на 9, так как на картинке только 9 строк
    for idx, player in enumerate(players[:9]):
        ...
        # Подбираем шрифт под ширину полосы
        font = _fit_font(text, BOX_WIDTH, FONT_BASE_SIZE)

        # anchor="mm" центрирует текст, белая обводка для красоты
        draw.text((x, y), text, font=font, fill="#1a4e5a", anchor="mm", stroke_width=1, stroke_fill="#ffffff")
...
async def init_pool() -> None:
    global pool
    try:
        pool = await asyncpg.create_pool(dsn=_normalize_dsn(DATABASE_URL), min_size=1, max_size=5, timeout=10.0)
        await create_tables()
...
async def create_tables() -> None:
    ...
    async with pool.acquire() as conn:
        ...
        """)
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_players_nickname ON players (LOWER(nickname));")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_players_team ON players (team_id);")
...
async def create_team(name: str, emoji_char: Optional[str], custom_emoji_id: Optional[str]):
    async with pool.acquire() as conn:
        return await conn.fetchrow("""
            ...
            RETURNING *;
        """, name, emoji_char, custom_emoji_id)
...
async def upsert_roster_player(nickname: str, number: int, team_id: int):
    async with pool.acquire() as conn:
        existing = await conn.fetchrow("SELECT id FROM players WHERE LOWER(nickname) = LOWER($1) AND number = $2;", nickname, number)
        if existing:
            await conn.execute("UPDATE players SET team_id = $1 WHERE id = $2;", team_id, existing["id"])
            return existing["id"], False
        row = await conn.fetchrow("INSERT INTO players (nickname, number, team_id) VALUES ($1, $2, $3) RETURNING id;", nickname, number, team_id)
        return row["id"], True

async def clear_team_roster_except(team_id: int, keep_player_ids: List[int]) -> None:
    async with pool.acquire() as conn:
        if keep_player_ids:
            await conn.execute("UPDATE players SET team_id = NULL WHERE team_id = $1 AND NOT (id = ANY($2::int[]));", team_id, keep_player_ids)
        else:
...
async def get_or_create_player(nickname: str, number: int):
    async with pool.acquire() as conn:
        row = await conn.fetchrow("SELECT * FROM players WHERE LOWER(nickname) = LOWER($1) AND number = $2;", nickname, number)
        if row:
            return row, False
        row = await conn.fetchrow("INSERT INTO players (nickname, number) VALUES ($1, $2) RETURNING *;", nickname, number)
        return row, True
...
async def apply_skater_stats(nickname: str, number: int, goals: int, assists: int, sign: str):
    ...
    async with pool.acquire() as conn:
        await conn.execute("""
            ...
            WHERE id = $4;
        """, mult * goals, mult * assists, mult * 1, row["id"])
    ...
    async with pool.acquire() as conn:
        await conn.execute("""
            ...
            SET saves = GREATEST(saves + $1, 0),
                ...
                is_goalkeeper = TRUE
            WHERE id = $4;
        """, mult * saves, mult * shots, mult * 1, row["id"])
    return row["id"], created

# ---------- ТОПЫ ИЗМЕНЕНЫ НА 9 ----------
async def top_scorers(limit: int = 9):
    async with pool.acquire() as conn:
        return await conn.fetch("""
            SELECT p.*, t.name AS team_name, t.emoji_char, t.custom_emoji_id, (p.goals + p.assists) AS points
            FROM players p
...
async def top_snipers(limit: int = 9):
    async with pool.acquire() as conn:
        return await conn.fetch("""
            SELECT p.*, t.name AS team_name, t.emoji_char, t.custom_emoji_id, (p.goals + p.assists) AS points
            FROM players p
...
async def top_assistants(limit: int = 9):
    async with pool.acquire() as conn:
        return await conn.fetch("""
            SELECT p.*, t.name AS team_name, t.emoji_char, t.custom_emoji_id, (p.goals + p.assists) AS points
            FROM players p
...
async def top_goalkeepers(limit: int = 9):
    async with pool.acquire() as conn:
        return await conn.fetch("""
...
SKATER_LINE_RE = re.compile(r"^\s*(\S+)\s*#\s*(\d+)\s+(\d+)\s+(\d+)\s+([+\-])\s*$")
GOALKEEPER_LINE_RE = re.compile(r"^\s*(\S+)\s*#\s*(\d+)\s+(\d+)\s*/\s*(\d+)\s+([+\-])\s*gk\s*$", re.IGNORECASE)
...
def parse_points_lines(text: str):
    ...
    for i, raw in enumerate(text.strip().splitlines(), start=1):
        ...
        if m_gk:
            results.append({"type": "gk", "nickname": m_gk.group(1), "number": int(m_gk.group(2)), "saves": int(m_gk.group(3)), "shots": int(m_gk.group(4)), "sign": m_gk.group(5)})
            continue
        m_sk = SKATER_LINE_RE.match(line)
        if m_sk:
            results.append({"type": "skater", "nickname": m_sk.group(1), "number": int(m_sk.group(2)), "goals": int(m_sk.group(3)), "assists": int(m_sk.group(4)), "sign": m_sk.group(5)})
            continue
        errors.append((i, line))
...
# ИЗМЕНЕНО НА ТОП-9
STAT_TITLES = {
    "points": "🏆 Топ-9 бомбардиров (голы + передачи)",
    "goals": "🎯 Топ-9 снайперов (голы)",
    "assists": "🅰️ Топ-9 ассистентов (передачи)",
}
STAT_LABELS = {
    "points": "очков",
    "goals": "голов",
...
def _strip_utf16_range(text: str, offset: int, length: int) -> str:
    u16 = text.encode("utf-16-le")
    return (u16[:offset * 2] + u16[(offset + length) * 2:]).decode("utf-16-le")
...
class Msg:
    ...
    def add_custom_emoji(self, fallback: Optional[str], custom_emoji_id: Optional[str]) -> "Msg":
        ...
        if custom_emoji_id and length:
            self.entities.append(MessageEntity(type="custom_emoji", offset=offset, length=length, custom_emoji_id=custom_emoji_id))
        return self
...
def _team_label_html(row) -> str:
    ...
    if row["custom_emoji_id"]:
        return f'<tg-emoji emoji-id="{row["custom_emoji_id"]}">{row["emoji_char"] or "🏒"}</tg-emoji> {t_name}'
    elif row["emoji_char"]:
...
def format_top_list(stat_key: str, players) -> str:
    ...
    for idx, p in enumerate(players, start=1):
        ...
        lines.append(f"{marker} <b>{nick} #{p['number']}</b> — {team_str}\n     ▸ <b>{value}</b> {label} (👕 Матчей: {p['matches_played']})")
    return "\n".join(lines)

def format_top_goalkeepers(players) -> str:
    title = "🥅 Топ-9 вратарей (% отраженных бросков)"
    if not players:
        return f"<b>{title}</b>\n\n📊 Пока нет данных."
    lines = [f"<b>{title}</b>\n"]
    for idx, p in enumerate(players, start=1):
        ...
        save_pct = p["save_pct"]
        nick = html.escape(p['nickname'])
        lines.append(f"{marker} <b>{nick} #{p['number']}</b> — {team_str}\n     ▸ <b>{save_pct}% ОБ</b> ({p['saves']}/{p['shots_against']} бросков, 👕 Матчей: {p['matches_played']})")
    return "\n".join(lines)
...
def main_menu_inline_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🏆 Топ-9 бомбардиров", callback_data="user:top:scorers")],
        [InlineKeyboardButton(text="🎯 Топ-9 снайперов", callback_data="user:top:snipers"), InlineKeyboardButton(text="🅰️ Топ-9 ассистентов", callback_data="user:top:assists")],
        [InlineKeyboardButton(text="🥅 Топ-9 вратарей", callback_data="user:top:goalkeepers")],
        [InlineKeyboardButton(text="👥 Состав команды", callback_data="user:teams"), InlineKeyboardButton(text="🔍 Найти игрока", callback_data="user:search")]
    ])

def back_to_main_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Вернуться в главное меню", callback_data="user:main_menu")]])
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(text, reply_markup=persistent_reply_kb())
    await message.answer("📍 <b>Главное меню:</b>", reply_markup=main_menu_inline_kb())
...
async def show_top_scorers(callback: CallbackQuery):
    await callback.answer()
    players = await top_scorers(9)
    if not players:
        await callback.message.edit_text("📊 Пока нет данных.", reply_markup=back_to_main_kb())
        return
    img_buffer = create_top_image("points", players)
    await callback.message.answer_photo(photo=BufferedInputFile(img_buffer.getvalue(), filename="top_scorers.png"), caption=format_top_list("points", players), reply_markup=back_to_main_kb())

@user_router.callback_query(F.data == "user:top:snipers")
async def show_top_snipers(callback: CallbackQuery):
    await callback.answer()
    players = await top_snipers(9)
    if not players:
        await callback.message.edit_text("📊 Пока нет данных.", reply_markup=back_to_main_kb())
        return
    img_buffer = create_top_image("goals", players)
    await callback.message.answer_photo(photo=BufferedInputFile(img_buffer.getvalue(), filename="top_snipers.png"), caption=format_top_list("goals", players), reply_markup=back_to_main_kb())

@user_router.callback_query(F.data == "user:top:assists")
async def show_top_assistants(callback: CallbackQuery):
    await callback.answer()
    players = await top_assistants(9)
    if not players:
        await callback.message.edit_text("📊 Пока нет данных.", reply_markup=back_to_main_kb())
        return
    img_buffer = create_top_image("assists", players)
    await callback.message.answer_photo(photo=BufferedInputFile(img_buffer.getvalue(), filename="top_assistants.png"), caption=format_top_list("assists", players), reply_markup=back_to_main_kb())

@user_router.callback_query(F.data == "user:top:goalkeepers")
async def show_top_goalkeepers(callback: CallbackQuery):
    await callback.answer()
    players = await top_goalkeepers(9)
    if not players:
        await callback.message.edit_text("📊 Пока нет данных.", reply_markup=back_to_main_kb())
        return
    img_buffer = create_top_image("goalkeepers", players)
    await callback.message.answer_photo(photo=BufferedInputFile(img_buffer.getvalue(), filename="top_goalkeepers.png"), caption=format_top_goalkeepers(players), reply_markup=back_to_main_kb())
...
async def show_team_roster_menu(callback: CallbackQuery):
    ...
    if not teams:
        await callback.message.edit_text("📊 <b>Команды пока не добавлены.</b> Загляните позже 🙂", reply_markup=back_to_main_kb())
        return
    ...
    if not roster:
        ...
        for p in roster:
            if p["is_goalkeeper"]:
                ...
                m.add_text(f"    👕 Матчей: {p['matches_played']} · 🛡 Отражено: {p['saves']}/{p['shots_against']} ({pct}%)\n\n")
            else:
                m.add_bold(f"⛸ {p['nickname']} #{p['number']}\n")
                m.add_text(f"    👕 Матчей: {p['matches_played']} · 🎯 Г: {p['goals']} · 🅰️ П: {p['assists']} · 🏆 О: {p['points']}\n\n")
...
async def find_player_prompt(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🔍 <b>Введите ник игрока (без номера):</b>", reply_markup=back_to_main_kb())
...
async def find_player_process(message: Message, state: FSMContext):
    ...
    if not query:
        await message.answer("⚠️ Пустой запрос. Попробуйте ещё раз через главное меню.", reply_markup=back_to_main_kb())
        return
    players = await search_players(query)
    if not players:
        await message.answer("😕 Игрок не найден. Проверьте ник и попробуйте снова.", reply_markup=back_to_main_kb())
        return
    if len(players) == 1:
        await _send_player_card(message, players[0])
        return
    await message.answer(f"🔍 Найдено несколько игроков ({len(players)}). Выберите нужного:", reply_markup=players_choice_kb(players))
...
async def adm_team_create_finish(message: Message, state: FSMContext):
    ...
    if not name:
        await message.answer("⚠️ Название не может быть пустым. Попробуйте снова.", reply_markup=admin_main_kb())
        return
    ...
    await message.answer(f"✅ <b>Команда «{esc_name}»{logo_note} успешно создана!</b>\n\n🛠 <b>Админ-панель:</b>", reply_markup=admin_main_kb())
...
async def adm_team_delete_menu(callback: CallbackQuery):
    ...
    if not teams:
        ...
        return
    await callback.answer()
    await callback.message.edit_text("🗑 <b>Выберите команду для удаления:</b>", reply_markup=teams_list_kb(teams, "adm:team:del", back_to="adm:teams"))
...
async def adm_team_delete_confirm(callback: CallbackQuery):
    ...
    esc_name = html.escape(team['name'])
    await callback.message.edit_text(f"🗑 <b>Команда «{esc_name}» удалена.</b>\nℹ️ Её игроки остались в базе, но теперь без привязки к команде.\n\n🛠 <b>Админ-панель:</b>", reply_markup=admin_main_kb())
...
async def adm_team_list(callback: CallbackQuery):
    ...
    if not teams:
        ...
        text = "📃 <b>Список всех команд:</b>\n\n" + "\n".join(f"• {t['emoji_char'] or '🏒'} {html.escape(t['name'])}" for t in teams)
...
async def adm_roster_menu(callback: CallbackQuery, state: FSMContext):
    ...
    if not teams:
        ...
        return
    await callback.answer()
    await callback.message.edit_text("📋 <b>Выберите команду, состав которой нужно обновить:</b>", reply_markup=teams_list_kb(teams, "adm:roster:team", back_to="adm:back"))
...
async def adm_roster_team_selected(callback: CallbackQuery, state: FSMContext):
    ...
    example = "miulio #9\nfrong #21\npetrov #64"
    esc_name = html.escape(team['name'])
    await callback.message.edit_text(f"📋 Команда: <b>{esc_name}</b>\n\n✏️ Отправьте список игроков, каждый на новой строке, в формате:\n<code>{example}</code>\n\n⚠️ <i>Этот список <b>полностью заменит</b> текущий состав команды.</i>", reply_markup=admin_cancel_kb("adm:roster"))
...
async def adm_roster_list_process(message: Message, state: FSMContext):
    ...
    if not entries:
        await message.answer("⚠️ Не удалось разобрать ни одной строки. Проверьте формат (ник #номер, каждый игрок на новой строке) и попробуйте снова.", reply_markup=admin_main_kb())
        return
    keep_ids: List[int] = []
    created, updated = 0, 0
    ...
    team = await get_team(team_id)
    esc_name = html.escape(team['name'])
    summary = f"✅ <b>Состав команды «{esc_name}» успешно обновлён!</b>\n\n🆕 Новых игроков: {created}\n🔄 Привязано существующих: {updated}"
    if errors:
...
async def adm_points_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("➕ <b>Отправьте статистику матча</b> — каждая строка отдельным игроком.\n\n⛸ <b>Полевые игроки:</b>\n<code>ник #номер голы передачи +/-</code>\n\n🥅 <b>Вратари:</b>\n<code>ник #номер отражено/всего +/- gk</code>\n\n📌 <b>Пример:</b>\n<code>{example}</code>\n\n🔹 «+» — добавить статистику и засчитать матч, «−» — вычесть.\n🔹 Если игрока нет в базе — он будет создан автоматически.", reply_markup=admin_cancel_kb("adm:back"))
...
async def adm_points_process(message: Message, state: FSMContext):
    ...
    if not results:
        await message.answer("⚠️ Не удалось разобрать ни одной строки. Проверьте формат и попробуйте снова.", reply_markup=admin_main_kb())
        return
    report_lines = []
    for r in results:
        nick = html.escape(r['nickname'])
        if r["type"] == "gk":
            _, created = await apply_goalkeeper_stats(r["nickname"], r["number"], r["saves"], r["shots"], r["sign"])
            tag = "🆕" if created else "✏️"
            report_lines.append(f"{tag} 🥅 <b>{nick} #{r['number']}</b>: {r['sign']} {r['saves']}/{r['shots']}")
        else:
            _, created = await apply_skater_stats(r["nickname"], r["number"], r["goals"], r["assists"], r["sign"])
            tag = "🆕" if created else "✏️"
            report_lines.append(f"{tag} ⛸ <b>{nick} #{r['number']}</b>: {r['sign']} Г:{r['goals']} П:{r['assists']}")
